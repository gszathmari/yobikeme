...
describe 'Helper: eventlogger (methods)', ->
  ...
  it 'fireDirections should return event', (done) ->
    ...
    expect(@stub.firstCall.args[1].keen.addons[0].name)
      .be.equal("keen:url_parser")
    expect(@stub.firstCall.args[1].keen.addons[0].input.url)
      .be.equal("yo_url")
    expect(@stub.firstCall.args[1].keen.addons[0].output)
      .be.equal("parsed_yo_url")
    expect(@stub.firstCall.args[1].keen.addons[1].name)
      .be.equal("keen:ip_to_geo")
    expect(@stub.firstCall.args[1].keen.addons[1].input.ip)
      .be.equal("user.ip_address")
    expect(@stub.firstCall.args[1].keen.addons[1].output)
